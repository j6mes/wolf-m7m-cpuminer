...
double GaussianQuad_N(const double x1, const double x2)
{
    double s=0.0;
    double x[6], w[6];
    //gauleg(a2, b2, x, w);
...
uint32_t sw_(int nnounce)
{
    double wmax = ((sqrt((double)(nnounce))*(1.+EPS1))/450+100);
    return ((uint32_t)(GaussianQuad_N(0., wmax)*(1.+EPS1)*1.e6));
}
    ...
    uint64_t max_nonce, unsigned long *hashes_done)
{
	...
	mpz_inits(magipi, magisw, bns[0], bns[1], bns[2], bns[3], bns[4], bns[5], bns[6], bns[7], NULL);
